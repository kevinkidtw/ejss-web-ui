...
export default function StagePanel() {
  ...
  // Auto-preview with 300ms debounce whenever view elements or model changes
  useEffect(() => {
  ...
  const handlePlay = () => {
    ...
    else { sendMsg('play'); setRunning(true); }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-gray-950 overflow-y-auto">
      ...
      {/* Variable bar */}
      {globalVars.length > 0 && (
        <div className="px-3 pt-1 pb-1.5 bg-gray-900 border-t border-gray-700 flex-shrink-0" title="顯示各模型變數的初始設定值（模擬執行後數值會動態更新於舞台內）">
